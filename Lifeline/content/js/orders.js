$(document).ready(function () {
    ...
    if (pageurl.indexOf("orders") != -1) {
        $("#ordertab-show").addClass('active');
    }
});
...
function setviewproductorders() {
    getproductorders(1, 10, 1, '');
    $(document).on("click", ".d-paging", function (event) {
        ...
        getproductorders(pageindex, pagesize, sortby, searchby);
    });
...
function getproductorders(pageindex, pagesize, sortby, searchby) {
    ...
    gl.ajaxreqloader(apiurl + "Product/GetAdminProductOrders", "get", { orgid: orgid, pgindex: pageindex, pgsize: pagesize, sortby: sortby, str: searchby, FromDate: from, ToDate: to }, function (response) {
...
function OrderItems(oid) {
    ...
    var row = '';
    var reccount = 0;
    gl.ajaxreqloader(apiurl + "Product/GetAdminProductOrderItems", "get", { oid: oid }, function (response) {
        //$('#cookinginstructions').val(response[0].cookingInstruction);
        $('#total').html(response[0].TotalBillDisplay);
        $('#OrderId').val(oid);
        if (response.length > 0) {
            $.each(response, function (i, item) {
                row += "<tr><td><img src='" + item.ProductImagePath + "' class='tblimgrw'/></td><<td>" + item.ProductName + "</td><td>" + item.DealAttributeValues + "</td><td>" + item.Qty + "</td><td>" + item.DisplayPrice + "</td></tr>";
            });
